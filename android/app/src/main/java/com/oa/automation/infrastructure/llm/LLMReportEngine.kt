...
12. 聆听·策划会优先保留多人讨论的时间线、匿名发言人、共识、分歧和待验证假设；音频来源只按原始资料填写，不声称已经直接接入微信、腾讯会议或电话系统

输出格式要求：
- 使用中文输出
- 结构清晰，便于阅读
- 一级标题必须概括本次记录的具体主题，不得只用会议类型名称或“会议纪要”作为标题。
- 模板的适用场景、写作定位、写作说明和输出约束只指导生成，不得复制到最终正文。
...
5. 下次会议建议议程"""
    ...
    fun buildUserPrompt(transcript: String, template: ReportTemplateConfig): String {
        ...
        val interactionSignalRule = if (meetingMode in setOf(
                ...
                MeetingMode.STANDUP,
                MeetingMode.LISTENING_PLANNING
            )
        ) {
            """
                可观察互动信号（如有证据）：
        ...
        val scenarioRule = when (meetingMode) {
            ...
            MeetingMode.FORUM -> """
                ...
                - 名录只填写明确到场或发言的人员；仅被提及、计划邀请、代为致意的未到场者不加入名录。姓名不明确者不加入，不输出占位行，不从照片推断身份，不在后文重复整段名录。
            """.trimIndent()
            MeetingMode.CUSTOM -> """
                自定义会议：
            ...
            MeetingMode.LISTENING_PLANNING -> """
                聆听·策划会：
                - 按实际收到的音频来源、时间轴和匿名发言人整理多人讨论，不把说话人编号当作真实身份。
                - 区分原始发言、共识、分歧、策划判断和待验证假设；没有证据的判断写“待确认”。
                - 单独列出有时间轴或发言人依据的语速变化、长停顿、打断、沉默和重复强调；不得把这些现象解释为情绪、意图或人格事实。
                - 不虚构微信、腾讯会议、电话或蓝牙设备已经完成平台级接入；来源只写原始记录明确提供的内容。
            """.trimIndent()
            MeetingMode.STUDY -> """
                研学考察游记式整理：
...
6. 首行标题概括本次记录的具体主题，不能沿用会议类型名称；适用场景、写作定位、输出约束等模板说明禁止写入最终正文。
