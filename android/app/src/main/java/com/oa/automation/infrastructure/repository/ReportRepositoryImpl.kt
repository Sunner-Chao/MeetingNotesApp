...
class ReportRepositoryImpl(
    ...
    override suspend fun save(report: Report): Result<Report> {
        return runCatching {
            ...
            reportDao.upsertReport(report.copy(ownerId = ownerId).toEntity())
            report
        }
    }
    ...
    override suspend fun findByMeetingId(meetingId: String): Result<Report?> {
        return runCatching {
            reportDao.findByMeetingId(meetingId, configDataStore.currentLocalWorkspaceAccountId() ?: "__anonymous__")?.toDomain()
        }
    }

    override suspend fun deleteByMeetingId(meetingId: String): Result<Unit> {
        return runCatching {
            reportDao.deleteByMeetingId(meetingId, configDataStore.currentLocalWorkspaceAccountId() ?: "__anonymous__")
        }
    }
