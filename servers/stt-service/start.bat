...
REM Use system Python instead of venv to avoid compatibility issues
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$env:STT_MODEL_ROOT='%STT_MODEL_ROOT%'; $env:STT_ENGINE='%STT_ENGINE%'; Start-Process -FilePath 'python' -ArgumentList @('-u','stt_server.py','--engine','%STT_ENGINE%','--model','%STT_MODEL%','--port','%STT_PORT%') -WorkingDirectory '%~dp0' -WindowStyle Minimized -RedirectStandardOutput '%STT_OUT_LOG%' -RedirectStandardError '%STT_ERR_LOG%'"
