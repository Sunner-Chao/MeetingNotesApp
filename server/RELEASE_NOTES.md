## 1.2.85 (Lite Android) · 首页比例优化与福利群页签直达

- 固定首页两张主入口的插画尺寸与文字安全区域，优化不同屏幕高度下的视觉比例，避免大卡片内部元素拉伸或相互遮挡。
- 福利群登录提醒的“去通知中心”现在直接打开通知中心“福利群”页签。
- 固定签名 Release APK `1.2.85/10285`（`com.oa.automation.light`）通过发布前包名、签名、非 debug、测试和 lint 校验。

## 1.2.84 (Lite Android) · 首页焦点入口与福利群登录提醒

- “即刻倾听”和“顷刻成稿”改为纵向弹性大入口，占据首页主要可用空间；插画、文字和操作按钮保持比例，避免拉伸失真。
- 最近记录收敛为底部导航栏上方的紧凑按钮，点击后以弹窗展示记录列表，保留打开、编辑和删除能力。
- 登录后自动提示通知中心福利群，支持“去通知中心”和“不再提醒”；偏好持久化保存，退出后重新登录按设置处理。
- 固定签名 Release APK `1.2.84/10284`（`com.oa.automation.light`）通过发布前包名、签名、非 debug、测试和 lint 校验。

## 1.2.83 (Lite Android) · 图片上传压缩与生成降级

- Agent 图片上传超过 `1MB` 时自动生成短生命周期 JPEG 上传副本，原始图片不改动。
- 图片压缩失败、批次超过 `32MB` 或可选图片请求被服务端拒绝时，自动降级为纯文字纪要，不再阻断整体生成流程。
- 失败重试会清理图片清单和图片定位锚点，避免最终纪要引用不存在的图号；必要错误仍保留明确失败提示。
- 固定签名 Release APK `1.2.83/10283`（`com.oa.automation.light`）通过发布前包名、签名、非 debug、测试和 lint 校验。

## 1.2.82 (Lite Android) · 首页主入口与最近记录简化

- 福利群入口统一移至通知中心“福利群”页签，首页不再重复展示福利宣传。
- “即刻倾听”和“顷刻成稿”恢复纵向大入口，突出两项核心操作。
- 最近记录改为紧凑单列列表，记录较多时在区域内滚动；筛选收敛为图标触发的轻量下拉菜单。
- 固定签名 Release APK `1.2.82/10282`（`com.oa.automation.light`）已通过包名、签名、非 debug、单元测试和 lint 校验；SHA-256：`6076be8b6822fe7dfeb124da8c27733c05936741cbc2150615e17db2b977fd0d`。

## 1.2.81 (Lite Android) · 福利群文案与账户入口调整

- Lite 通知中心恢复 1.2.79 的会议通知页，隐藏活动与福利群标签；首页保留群入口及既有问卷流程。
- 首页标题改为“加入智悟本07福利群”，说明为“群里领福利，有问题一起聊，新活动也会在这里告诉你”。
- “我的”移除“企微群与问卷”；“删除账户与数据”移入个人资料，保留确认、忙碌状态和失败提示，删除成功清理导航栈并返回登录。

## 1.2.80 (Lite Android) · 企微群与用户调研

- Lite 首页新增企微群宣传卡片；“我的”独立显示企微群与问卷入口，通知中心恢复活动、福利群和审核消息，同时保持社区入口隔离。
- 群主名片不再裁切，放大、保存和游客登录申请均可用；小屏可滚动，游客刷新可重试，回到前台更新审核状态。
- 入群问卷补充使用场景、记录频率、整理方式、核心痛点、付费偏好及建议；以点选为主，选填项可跳过，提交失败保留内容。
- 后台审核卡片完整展示调研字段；复用现有申请接口，继续在审核通过后开放群二维码。
- 已固定签名发布 `1.2.80/10280`，Lite OTA 仅保留 `10280/10279`，Pro 仍为 `10264`；公网 APK SHA-256 与本地一致。Android 单元测试 346 项及服务端账户测试 24 项通过，Release lint 无错误；ADB 无可用设备，未执行模拟器截图验收。

## 1.2.70 (Light Enjoy Android)

- Lite 内测阶段暂时关闭本地 STT 与模型切换入口；保留构建参数 `meetingnotesLiteLocalSttEnabled`，后续可在不改 UI 的情况下重新开启。
- Room 会议与纪要增加 `ownerId` 及索引，查询、保存和删除均绑定当前账户；账户切换时清理本地工作区与私有媒体缓存，避免跨账户串用。
- 账户 STT 令牌随当前会话更新或清除，不复用上一账户令牌；云端 `account_meetings` 继续由服务端 `user_id` 隔离。
- 固定签名 Android Release `1.2.70/10270`（包名 `com.oa.automation.light`）通过 Lite OTA 发布，服务器仅保留 `10270` 与上一版 `10269`。

## 1.2.59-20260907093151456 · 首页统一两列栅格

- 首页收敛到单一两列栅格：`即刻倾听`／`顷刻成稿` 入口改为与记录卡同宽同 16dp 圆角的并排磁贴，去掉磁贴内嵌的冗余按钮，共用 `gridGutter` 一个间距来源。
- `最近记录` 改为双列卡片：新增波形（按会议 ID 生成、稳定不随重组变化）与文稿缩略图带、状态徽标、类型胶囊，`待完善` 保留“继续完善”入口；筛选卡改为圆角矩形胶囊加行间细线，并常驻显示“N 条记录 · M 条待完善”。
- 会议时长超过一小时改用小时显示（`360分钟` → `6小时`），一小时以内行为不变。
- 修复设置页底部“当前版本／检查更新／恢复默认设置”被挤出屏幕截断的问题：内容区改为可滚动，操作行固定在 `bottomBar` 并预留导航栏内边距。
- 浅色主题为卡片与磁贴补柔和阴影，深色主题不加阴影。
- 本次正式发布：Pro `1.2.64/10264`（`com.oa.automation`），Lite `1.2.67/10267`（`com.oa.automation.light`）。每个通道仅保留最新版本和上一版本。

## 1.2.59-20260905101853107 · Pro/Lite OTA 通道隔离

- 正式产品名称统一为 `智悟本(Pro)`（社交版）与 `智悟本(Lite)`（轻享版）。
- Android Pro 与 Lite 使用独立的产品标识、包名、metadata endpoint 和 APK 目录；服务端 metadata 增加 `product_edition`、`product_name` 与 `application_id`。
- Android 客户端严格校验 metadata 的产品标识，endpoint 配置错误时拒绝展示另一产品的更新提醒。
- 本次正式发布：Pro `1.2.63/10263`（`com.oa.automation`），Lite `1.2.66/10266`（`com.oa.automation.light`）。每个通道仅保留最新版本和上一版本。

## 1.2.65 (Light Enjoy Android)

- 轻享版“顷刻成稿”页重做为空态黑板工作流：展示音频转文字到会议纪要的六步流程，并可按会议书签切换对应效果图。
- 新增纯文本手动输入入口，统一顶栏、导入入口、文本面板和生成按钮的手绘风格；画布跟随系统深浅色，深色效果图自动压暗。
- 固定签名 Android Release `1.2.65/10265`（包名 `com.oa.automation.light`）通过 OTA 轻享版独立通道发布，服务器仅保留 `10265` 与 `10264`。

## 1.2.58

- 修复流式 WebSocket 重连后的实时转录时间轴：客户端以 PCM 产生位置计算连接起点，累加器仅应用一次偏移，续录内容保持正确顺序。
- 统一腾讯云稀疏说话人编号与界面、持久化标签，服务切换时保留同一会议的编号，新会议重新开始编号。
- 统一 Room 转写查询按录音时间排序，并兼容旧数据和历史说话人前缀格式。
- 增加重连偏移、腾讯云说话人映射和历史文本渲染回归测试。
- 固定签名 Android Release 版本号升级为 `1.2.58/10258`；发布后 OTA 仅保留 `10258` 与 `10257`。

## 1.2.57

- Android 实时转录、会议纪要、研学阶段稿和账户同步统一使用带说话人标签的结构化内容，兼容旧数据并避免重复前缀。
- 修正续录时转写时间轴偏移：新片段从既有会议总时长开始计时，暂停/恢复与多段音频合并后的内容保持正确顺序。
- 增强流式说话人分段在 WebSocket 重连、版本修订和相邻同一说话人场景下的保留与合并，并补充回归测试。
- 设置页为“语音增强”和“说话人分离”补充简洁的中文说明。
- 固定签名 Android Release 版本号升级为 `1.2.57/10257`；发布后 OTA 仅保留 `10257` 与 `10256`。
...
- 会议类型书签折叠态使用简短名称（如“推演会”），白字从上到下正向竖排，避免旋转阅读和窄栏裁切。
- 固定签名 Android Release 版本号升级为 `1.2.79/10279`，发布后 OTA 仅保留 `10279` 与 `10278`。
## 1.2.65 (Pro Android) · V100 本地 STT 主链路

- FunASR Paraformer Large 在线模型部署到 V100，实时预览与文件转写分别使用 `cuda:0` / `cuda:1`。
- Android Pro 默认本地入口迁移到 IP 证书地址，避免部分 IPv4 网络对 `lstwin.space` 的 TLS/DNS reset；腾讯云 `/stt-cloud` 保持兜底。
- V100 WebSocket 兼容 Android 请求头鉴权和浏览器 `authenticate`，暂停/继续及停止尾句刷新通过回放验收。
- 固定签名 APK `1.2.65/10265` 已通过 OTA 原子发布，服务器仅保留 `10265` 与 `10264`。
## 1.2.86 (Lite Android) · 时间线转写与说话人片段
