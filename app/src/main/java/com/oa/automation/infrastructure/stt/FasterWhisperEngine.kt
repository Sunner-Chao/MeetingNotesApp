...
class FasterWhisperEngine(
        ...
        try {
            ...
            if (!response.isSuccessful) {
                return@withContext Result.failure(
                    IOException("STT request failed: ${response.code} ${response.message}")
                )
            }

            val responseBody = response.body?.string() ?: return@withContext Result.failure(
                IOException("Empty response from STT service")
            )
    ...
    override fun getDisplayName(): String = "Faster-Whisper (本地)"
...
object STTServiceClient {
     ...
     */
    fun testConnection(endpoint: String): Result<Unit> {
        return try {
            val request = Request.Builder()
                .url("$endpoint/health")
                .get()
