...
class OllamaEngine(
    ...
    override suspend fun generateReport(transcript: String, template: ReportTemplateConfig): Result<ReportData> = withContext(Dispatchers.IO) {
        try {
            ...
            // Parse structured report from LLM output
    ...
    override suspend fun chat(messages: List<ChatMessage>): Result<String> = withContext(Dispatchers.IO) {
        try {
            val endpoint = config.localEndpoint
            val model = config.localModel
